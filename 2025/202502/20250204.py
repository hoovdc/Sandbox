...
# File paths - cross-platform compatible and configurable
CURRENT_DIR = Path(__file__).parent
CONFIG_DIR = CURRENT_DIR / 'config'  # New directory for sensitive configs
DATA_DIR = CURRENT_DIR / 'data'      # New directory for data storage
...
def check_environment():
    """Verify all required environment variables and config files exist."""
    required_dirs = [CONFIG_DIR, DATA_DIR]
    required_files = [CREDENTIALS_PATH, NOTION_CONFIG_PATH]
    ...
    for file_path in required_files:
        if not file_path.exists():
            raise FileNotFoundError(
                f"Missing required config file: {file_path}\n"
...
def get_notion_headers():
    """Get Notion headers from config file."""
    try:
        with open(NOTION_CONFIG_PATH, 'r') as file:
            config = json.load(file)
        
        notion_key = config.get('NOTION_SECRET_KEY')
        database_id = config.get('NOTION_DATABASE_ID')
        
        if not notion_key or not database_id:
            raise ValueError("Missing required Notion configuration values")
        ...
        raise ValueError(
            "Error loading Notion configuration. "
            "Please ensure notion_config.json is properly set up."
        ) from e
...
def setup_database():
    """Setup SQLite database with proper permissions."""
...
def process_notion_data(results):
    processed_data = []
    for page in results['results']:
        props = page['properties']
        if 'Name' in props and props['Name']['title']:
            name = props['Name']['title'][0]['text']['content'].lower()
            if not any(word in name for word in QUESTIONABLE_WORDS):
