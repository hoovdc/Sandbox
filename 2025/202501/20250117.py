...
def process_calendar_events(events):
    ...
    daily_events = {}  # Dictionary of date -> list of events
    central = pytz.timezone('US/Central')
    
    # First, organize events by date
    for event in events:
        if 'attendees' in event:
            if any(attendee.get('self') and attendee.get('responseStatus') == 'declined' 
                   for attendee in event['attendees']):
        ...
        if not questionable:
            ...
            if date not in daily_events:
                daily_events[date] = []
            daily_events[date].append(event_info)
    ...
    for date, day_events in daily_events.items():
        adjusted_events = adjust_overlapping_events(day_events)
        total_hours = sum(event['duration'] for event in adjusted_events)
        daily_totals[date] = int(total_hours * 60)  # Convert hours to minutes
...
def main():
    ...
    # Temporarily reduce date range for faster testing
    start_date = today - datetime.timedelta(days=10)  # Change back to datetime.date(2024, 8, 5) later
    ...
    while current_date <= end_date:
        print(f"Processing {current_date}...", end='\r')  # Progress indicator
