...
def write_to_gsheet(rows, output, delimiter):
...
def main():
    ...
    if args.output:
        ...
        #if there is no output file name specified, generate using the output date
        output = "screentime_%s.csv" % (datetime.datetime.now().strftime("%Y%m%d"))
        #output = StringIO() #use this to name the output CSV file after this .PY file (e.g. screen_time.csv)
